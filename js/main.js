(function () {
  ...
  var counterWayPoint = function () {
    if ($('#rajat-counter').length > 0) {
      $('#rajat-counter').waypoint(
        function (direction) {
  ...
  var burgerMenu = function () {
    $('.js-rajat-nav-toggle').on('click', function (event) {
  ...
  var mobileMenuOutsideClick = function () {
    $(document).click(function (e) {
      var container = $('#rajat-aside, .js-rajat-nav-toggle');
      if (!container.is(e.target) && container.has(e.target).length === 0) {
        if ($('body').hasClass('offcanvas')) {
          $('body').removeClass('offcanvas');
          $('.js-rajat-nav-toggle').removeClass('active');
        }
      }
    });
  ...
  var sliderMain = function () {
    $('#rajat-hero .flexslider').flexslider({
      animation: 'fade',
      slideshowSpeed: 5000,
      directionNav: true,
      start: function () {
...
document
  .getElementById('download-resume-btn-for-mobile')
  .addEventListener('click', function () {
    var link = document.createElement('a');
